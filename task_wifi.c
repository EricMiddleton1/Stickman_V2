...
int wifi_start(char* ssid, char *psk, uint8_t pskLen) {
	...
	sl_WlanSet(SL_WLAN_CFG_AP_ID, WLAN_AP_OPT_PASSWORD, pskLen, psk);						//PSK
		...
		while(_apState == CONNECTED_WITH_IP) {
			...
			while(_apState == CONNECTED_WITH_IP && (retval > 0 || retval == SL_EAGAIN)) {
				...
				if(retval > 0) {
					...
					Communicator_processBuffer(recvBuffer, retval);
				}
